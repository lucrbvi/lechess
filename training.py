...
def batches(dataset, batch_size: int, window_size: int, phase: str, device: str | None):
    batch = []
    for sample in dataset:
        ...
        for i in range(len(moves) - window_size + 1):
            if phase == 'bc':
                if rewards is None:
                    batch.append((
                        boards[i:i + window_size + 1],
                        moves[i:i + window_size],
                        np.full(window_size, sample['result'] * 2 - 1, dtype=np.float32),
                    ))
                else:
                    if not np.isfinite(rewards[i:i + window_size]).all():
                        continue
                    batch.append((boards[i:i + window_size + 1], moves[i:i + window_size], rewards[i:i + window_size]))
            else:
            ...
            if len(batch) == batch_size:
                ...
                if phase == 'bc':
                    out['rewards'] = Tensor(np.stack([x[2] for x in batch]), device=device).clone().realize()
                yield out
                batch = []
...
def metric_values(metrics: dict[str, Tensor]) -> dict[str, float]:
    return {key: value.realize().item() for key, value in metrics.items()}
...
def run_eval(phase: str, dataset, encoder, predictor, heads, args) -> dict[str, float]:
    ...
    with Tensor.train(False):
        for step, batch in enumerate(batches(dataset, args.batch_size, window, phase, args.device), start=1):
            ...
            metrics.append(metric_values(values))
...
def train(args):
    ...
    heads = WorldModelHeads(dim=args.dim, mtp_steps=args.mtp_steps, reward_bins=args.reward_bins) if args.phase == 'bc' else None
    ...
    objects = {'encoder.': encoder, 'predictor.': predictor}
    ...
    train_step = TinyJit(bc_step if args.phase == 'bc' else world_step) if not args.no_jit else bc_step if args.phase == 'bc' else world_step
    for n, batch in enumerate(batches(train_dataset, args.batch_size, window, args.phase, args.device), start=1):
        if args.phase == 'world':
            metrics = metric_values(train_step(batch['boards'], batch['moves']))
        else:
            metrics = metric_values(train_step(batch['boards'], batch['moves'], batch['rewards']))
